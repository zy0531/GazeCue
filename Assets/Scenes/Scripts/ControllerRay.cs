...
public class ControllerRay : MonoBehaviour
{
    ...
    void Update()
    {
        // if ControllerRayLayerMask is Nothing
        if (ControllerRayLayerMask == 0)
            // version 1
            AlignLineRenderer(rend, teleportPointer.position, teleportPointer.forward); 
        else
            // version 2
            AlignLineRenderer(rend, teleportPointer.position, teleportPointer.forward, ControllerRayLayerMask);
    }
    ...
    // version 1: return void (not check of hit)
    ...
    void DrawControllerRay(LineRenderer rend, Vector3 originposition, Vector3 direction, LayerMask layers)
    {
        Ray ray = new Ray(originposition, direction);
        RaycastHit hit;
        Vector3 endPosition = originposition + (10f * direction);
